...
void scanFace(char line[], int numSides) {
  ...
  //if (numSides == 3) {
    //sscanf_s (line, "f %i/%i/%i %i/%i/%i %i/%i/%i", &v1, &t1, &n1, &v2, &t2, &n2, &v3, &t3, &n3);
  //} else {
    sscanf_s (line, "f %i/%i/%i %i/%i/%i %i/%i/%i %i/%i/%i", &v1, &t1, &n1, &v2, &t2, &n2, &v3, &t3, &n3, &v4, &t4, &n4);
  //}
  struct face f;
